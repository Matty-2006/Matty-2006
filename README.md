![Header Banner](https://raw.githubusercontent.com/Matty-2006/Matty-2006/main/header.jpg)

# Mateo Rodríguez 👋
### Estudiante de Desarrollo de Software @ EPN (ESFOT) 🎓 | Programador Junior 💻

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mateo-rodr%C3%ADguez-81b8332a4/)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mateorodriguezxd106@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Matty-2006)

---

### 🚀 Sobre Mí

* 🎓 **Educación:** Estudiante de Desarrollo de Software en la Escuela Politécnica Nacional (ESFOT).
* 💻 **Lenguajes:** Java, JavaScript, Python, SQL, NoSQL.
* 🛠️ **Frameworks & Librerías:** Spring Boot, React, Node.js, Express, JavaFX.
* 📊 **Análisis de Datos & ETL:** KNIME, Power BI, JasperReports, Optimización de Bases de Datos.
* ⚙️ **Herramientas & DevOps:** Git, GitLab, Docker, Git LFS.
* 🎯 **Enfoque Actual:** Diseño de aplicaciones escalables, arquitectura MVC, APIs REST y gestión eficiente de datos.

---

### ⚡ Proyectos en Desarrollo Activo

> ⚡ **CNM App:** Plataforma de gestión de tutorías académicas desarrollada con Spring Boot y JavaFX.
> ⚡ **NetCourse:** Plataforma web interactiva para la enseñanza de conceptos de redes de computadoras.
> ⚡ **Frutpulp:** Aplicación para control de inventarios, catálogo y logística de distribución.

---

### 🛠️ Tecnologías y Ecosistema

| Categoría | Tecnologías y Herramientas |
| :--- | :--- |
| **Lenguajes & Backend** | ![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openapi-initiative&logoColor=white) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black) ![Node.js](https://img.shields.io/badge/Node.js-43853D?style=flat-square&logo=node.js&logoColor=white) ![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring-boot&logoColor=white) |
| **Frontend & Interfaz** | ![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB) ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white) ![JavaFX](https://img.shields.io/badge/JavaFX-FF0000?style=flat-square&logo=openjdk&logoColor=white) |
| **Bases de Datos & Analytics** | ![SQL](https://img.shields.io/badge/SQL-003B57?style=flat-square&logo=postgresql&logoColor=white) ![NoSQL](https://img.shields.io/badge/NoSQL-4EA94B?style=flat-square&logo=mongodb&logoColor=white) ![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat-square&logo=powerbi&logoColor=black) ![KNIME](https://img.shields.io/badge/KNIME-FFD700?style=flat-square&logo=knime&logoColor=black) |
| **Herramientas & Control** | ![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white) ![GitLab](https://img.shields.io/badge/GitLab-FC6D26?style=flat-square&logo=gitlab&logoColor=white) ![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white) ![Git LFS](https://img.shields.io/badge/Git_LFS-000000?style=flat-square&logo=git&logoColor=white) |

---

### 💻 Proyectos Destacados

| Proyecto | Descripción | Tecnologías |
| :--- | :--- | :--- |
| **Sistema CNM** | Gestión de roles, verificación automatizada por correo y backend en Spring Boot. | `Java` `Spring Boot` `JavaFX` `SQL` |
| **NetCourse** | Plataforma educativa interactiva con modo claro/oscuro y componentes de chat. | `React` `JavaScript` `CSS` `Node.js` |
| **Pipeline KNIME** | Flujo de procesamiento ETL para limpieza masiva de datos y normalización de fechas. | `KNIME` `Python` `SQL` |
| **Frutpulp Suite** | Plataforma web de control de existencias, trazabilidad e inventario. | `JavaScript` `Node.js` `Databases` |

---

### 📈 Lenguajes Más Usados (Actualización Automática)

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Matty-2006&theme=2077" alt="Lenguajes por Repositorio" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Matty-2006&theme=2077" alt="Lenguajes por Commits" />
</p>

---

### 🐍 Racha de Contribuciones

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Matty-2006&theme=tokyonight&hide_border=true" alt="Streak Stats" />
</p>

---

> *"El código limpio siempre parece escrito por alguien a quien le importa."*
